...
| 2026-06-06 | prp3_savgol_eval | LightGBM (R1, 18 features) | Post-processing: Savgol smoothing + TVT clipping (OOF evaluation) | OOF raw `14.2187`. Savgol w=31 p=2: `14.2123` (−0.0064). TVT clip [9852,12860]: `14.2208` (+0.002). Savgol w=17 p=3: `14.2135` (−0.0052). | Not submitted | `--eval-postproc` on R1 | **Savgol HELPS**: all smoothed configs beat raw, w=31 p=2 best. Clipping HURTS (0.2% outside range, minimal effect but slight degradation). TVT range p0.1-p99.9 = [9852, 12860]. Per-well viz: 3/3 wells improved, raw max jumps 1.6-4.1 ft (not formation boundaries — noise). **Decision: PROMOTE Savgol w=31 p=2. REJECT TVT clipping.** |
